...
def viz_kitti_video():
    ...
    for _ in range(len(dataset)):
        ...
        draw_lidar(pc)
        # raw_input() là một hàm trong Python 2 để nhận input từ người dùng
        # Trong Python 3, nó đã được thay thế bằng input()
        # Ở đây nó được dùng để tạm dừng chương trình cho đến khi người dùng nhấn Enter
        # để có thể xem kết quả visualization
        try:
            input()  # Python 3
        except NameError:
            input()  # Python 2
...
def show_lidar_with_depth(
    ...
    objects,
    calib,
    fig=None,  # Keep for compatibility but not used with Open3D
    img_fov=False,
    img_width=None,
    img_height=None,
    objects_pred=None,
    ...
    import open3d as o3d
    from Point_Cloud_3D_Object_Detection.visualizer.viz_utils import draw_lidar_simple, draw_lidar, draw_gt_boxes3d
    ...
    # Get geometries from draw_lidar
    geometries = draw_lidar(pc_velo, pc_label=pc_label)
    ...
    if depth is not None:
        ...
        # Create depth point cloud
        depth_pcd = o3d.geometry.PointCloud()
        depth_pcd.points = o3d.utility.Vector3dVector(depth_pc_velo[:, :3])
        depth_pcd.paint_uniform_color([1, 1, 1])  # White
    ...
    # Draw bounding boxes for ground truth objects
    for obj in objects:
        if obj.type == "DontCare":
            continue
        ...
        #TODO: change the color of boxes
        if obj.type == "Car":
            box_geometries = draw_gt_boxes3d([box3d_pts_3d_velo], color=(0,1,0), label=obj.type)
        elif obj.type == "Pedestrian":
            box_geometries = draw_gt_boxes3d([box3d_pts_3d_velo], color=(0,1,1), label=obj.type)
        elif obj.type == "Cyclist":
            box_geometries = draw_gt_boxes3d([box3d_pts_3d_velo], color=(1,1,0), label=obj.type)
        elif  obj.type=='Person_sitting':
            box_geometries = draw_gt_boxes3d([box3d_pts_3d_velo], color=(1,2,0), label=obj.type)
        elif obj.type == 'Van':
            box_geometries = draw_gt_boxes3d([box3d_pts_3d_velo], color=(0,0,1), label=obj.type)
        elif obj.type == 'Truck':
            box_geometries = draw_gt_boxes3d([box3d_pts_3d_velo], color=(1,0,1), label=obj.type)
        elif obj.type == 'Tram':
            box_geometries = draw_gt_boxes3d([box3d_pts_3d_velo], color=(0,1,1), label=obj.type)
        elif obj.type == 'Misc':
            box_geometries = draw_gt_boxes3d([box3d_pts_3d_velo], color=(1,0,0), label=obj.type)
        else:
            box_geometries = draw_gt_boxes3d([box3d_pts_3d_velo], color=(0, 1, 0), label=obj.type)
    ...
    if objects_pred is not None:
        ...
        for obj in objects_pred:
            ...
            print("box3d_pts_3d_velo:")
            print(box3d_pts_3d_velo)
            box_geometries = draw_gt_boxes3d([box3d_pts_3d_velo], color=color)
            geometries.extend(box_geometries)
            ...
            # Create line for heading arrow
            line = o3d.geometry.LineSet()
            points = o3d.utility.Vector3dVector([[x1, y1, z1], [x2, y2, z2]])
...
def show_lidar_with_boxes(
    ...
    for obj in objects:
        ...
        if depth is not None:
            ...
            dep_pcd.points = o3d.utility.Vector3dVector(dep_pc_velo[:, :3])
            dep_pcd.paint_uniform_color([1, 1, 1])  # White
            geometries.append(dep_pcd)
    ...
    for data_idx in range(len(dataset)):
        ...
        if args.show_lidar_with_depth:
            ...
            show_lidar_with_depth(
                ...
                objects,
                calib,
                None,  # fig parameter no longer needed
                args.img_fov,
                img_width,
                img_height,
                objects_pred,
