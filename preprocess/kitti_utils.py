...
class Object2D(object):
    def __init__(self, label_file_line):
        ...
        self.occlusion= float(data[2])
        self.box2d = np.array([data[3], data[4], data[5], data[6]])

    def print_object(self):
        print(f'type: {self.type}, truncation: {self.truncation}, occlusion: {self.occlusion}')
        
        print(f'2d box: {self.box2d}, x0: {self.box2d[0]}, y0: {self.box2d[1]}, x1: {self.box2d[2]}, y1: {self.box2d[3]}')
...
class Object3D(object):
    def __init__(self, label_file_line):
        ...
        self.alpha= float(data[3])

        self.xmin= float(data[4])
        self.ymin= float(data[5])
        self.xmax= float(data[6])
        self.ymax= float(data[7])
        self.box2d = np.array([self.xmin, self.ymin, self.xmax, self.ymax])


        self.h= float(data[8])
        self.w= float(data[9])
        self.l= float(data[10])

        self.t= (float(data[11]), float(data[12]), float(data[13]))
        self.ry= float(data[14])

    
    def estimate_difficuty(self):
...
def draw_box3d_on_top(
    ...
    for n in range(num):
        ...
        if is_gt:
            ...
            color = cv2.heat_map_rgb(0.0, 1.0, scores[n]) if scores is not None else 255
            startx = 85
